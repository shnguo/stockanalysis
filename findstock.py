...
df_result['今日主力净流入-净额'].fillna(-1,inplace=True)
df_result['今日主力净流入-净额'] = df_result['今日主力净流入-净额'].replace('-',-1)
df_result['今日主力净流入-净额'] = df_result['今日主力净流入-净额'].astype('float64')
...
df_result2 = df_result[(df_result.MA5.round(2) >= df_result.MA5_1.round(2))
                       ...
                       (df_result.MA60.round(2) >= df_result.MA60_1.round(2)) &
                       (df_result.涨跌幅>0.0) & 
                       (df_result['今日主力净流入-净占比']>0.0)
                       ].sort_values(
                           [
                               'MA60_score',
                               'MA30_score',
